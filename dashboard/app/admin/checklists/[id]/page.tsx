...
interface Item { id: string; label: string; category: string; }
...
export default function EditChecklistPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    fetch(`${API}/checklists/${params.id}`)
      ...
      .catch(() => { setError('Failed to load checklist'); setLoading(false); });
  }, [params.id]);
  ...
  if (loading) return <div className="min-h-screen bg-gray-950 text-white flex items-center justify-center"><p className="text-gray-400">Loading...</p></div>;
        ...
        <div className="space-y-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <label className="text-sm text-gray-400">Items ({items.length})</label>
