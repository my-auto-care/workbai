...
class _PhotoMarker {
  ...
  String? s3Key;
  bool uploading;
  _PhotoMarker({required this.localPath, required this.timestamp, required this.recordingOffset, this.s3Key, this.uploading = true});
}
...
class _TranscriptScreenState extends State<TranscriptScreen> {
  ...
  Future<void> _startSegment() async {
    ...
    try {
      ...
      _currentSegmentPath = '${dir.path}/seg_${widget.sessionId}_$_segmentIndex.aac';
      _segmentIndex++;
      _segmentStartTime = DateTime.now();

      await _recorder.startRecorder(
        toFile: _currentSegmentPath,
        codec: Codec.aacADTS,
        bitRate: 64000,
        sampleRate: 16000,
        numChannels: 1,
      ...
      _ticker = Timer.periodic(const Duration(seconds: 1), (_) {
        if (mounted && _state == _State.recording) setState(() => _elapsedSeconds++);
      });
  ...
  Future<void> _pause() async {
    ...
    setState(() { _state = _State.paused; _log.add('⏸️ Paused'); });
  }
  ...
  Duration get _currentOffset {
    final extra = _segmentStartTime != null ? DateTime.now().difference(_segmentStartTime!) : Duration.zero;
    return _totalRecorded + extra;
  }
  ...
  Future<void> _insertPhoto() async {
    ...
    if (wasRecording) {
      _ticker?.cancel();
      final path = await _recorder.stopRecorder();
      if (path != null && File(path).existsSync() && File(path).lengthSync() > 0) _audioSegments.add(path);
      if (_segmentStartTime != null) {
        _totalRecorded += DateTime.now().difference(_segmentStartTime!);
        _segmentStartTime = null;
      }
    ...
    final XFile? photo = await picker.pickImage(source: ImageSource.camera, imageQuality: 60, maxWidth: 1280, maxHeight: 960);
    if (wasRecording) await _startSegment();
    if (photo == null) return;
    final marker = _PhotoMarker(localPath: photo.path, timestamp: DateTime.now(), recordingOffset: offset);
    setState(() { _photos.add(marker); _log.add('📷 Photo at ${_formatDuration(offset)} — uploading...'); });
  ...
  void _uploadPhoto(XFile photo, _PhotoMarker marker) {
    () async {
      try {
        final uploadData = await ApiService().getUploadUrl(sessionId: widget.sessionId, filename: 'photo_${DateTime.now().millisecondsSinceEpoch}.jpg');
        final bytes = await File(photo.path).readAsBytes();
        final resp = await http.put(Uri.parse(uploadData['upload_url']), headers: {'Content-Type': 'image/jpeg'}, body: bytes);
  ...
  Future<void> _complete() async {
    _ticker?.cancel();
    setState(() { _state = _State.processing; _log.add('⏹️ Stopped — sending to AI...'); });
    try {
      if (_recorder.isRecording) {
        final path = await _recorder.stopRecorder();
        if (path != null && File(path).existsSync() && File(path).lengthSync() > 0) _audioSegments.add(path);
      }
      if (_audioSegments.isEmpty) throw Exception('No audio recorded');
      final primaryAudio = _audioSegments.reduce((a, b) => File(a).lengthSync() >= File(b).lengthSync() ? a : b);
      ...
      final result = await ApiService().processTranscript(sessionId: widget.sessionId, audioPath: primaryAudio, photoMarkers: photoOffsets);
      ...
      if (mounted) Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => SessionScreen(sessionId: widget.sessionId)));
    } catch (e) {
      setState(() { _error = e.toString(); _state = _State.error; });
    }
  }
      ...
      body: SafeArea(child: Column(children: [
        Container(
          ...
          child: Row(children: [
            ...
            const Spacer(),
            Text(_formatDuration(_totalRecorded + (isRecording ? Duration(seconds: _elapsedSeconds) : Duration.zero)),
            ...
            child: ListView.builder(
              ...
              itemBuilder: (_, i) {
                ...
                return Container(
                  ...
                  width: 76,
                  decoration: BoxDecoration(borderRadius: BorderRadius.circular(8), border: Border.all(color: p.uploading ? const Color(0xFFFBBF24) : const Color(0xFF10B981))),
                  child: Stack(children: [
                    ClipRRect(borderRadius: BorderRadius.circular(7), child: Image.file(File(p.localPath), width: 76, height: 76, fit: BoxFit.cover)),
                    Positioned(bottom: 0, left: 0, right: 0, child: Container(
                      padding: const EdgeInsets.symmetric(vertical: 2),
                      decoration: const BoxDecoration(color: Colors.black54, borderRadius: BorderRadius.vertical(bottom: Radius.circular(7))),
                      child: Text(_formatDuration(p.recordingOffset), textAlign: TextAlign.center, style: const TextStyle(color: Colors.white, fontSize: 9)),
                    )),
                    if (p.uploading) const Positioned.fill(child: Center(child: SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)))),
                  ]),
                );
              },
            ),
        ...
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (_, i) => Padding(
              ...
              child: Text(_log[i], style: const TextStyle(color: Color(0xFF9CA3AF), fontSize: 14, height: 1.5)),
            ),
          ),
        ),
        if (isProcessing)
            ...
            child: Row(children: [
              Expanded(
                child: ElevatedButton.icon(
                  ...
                  label: Text(isRecording ? 'Pause' : 'Resume', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 15)),
                ),
              ),
        ...
        if (_state == _State.error)
          Container(
            ...
            child: Column(children: [
              ...
              ElevatedButton(onPressed: _initRecorder, child: const Text('Retry')),
            ]),
          ),
      ])),
    );
